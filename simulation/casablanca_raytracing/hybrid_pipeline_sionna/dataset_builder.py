...
class DatasetBuilder:
    ...
    def _build_row(
        ...
        all_rays = []
        for meas in top_neighbors:
            if hasattr(meas, "ray_paths"):
                all_rays.extend(meas.ray_paths)
        ...
        return {
            ...
            "ray_tx_ids": json.dumps([r.tx_id for r in all_rays]),
            "ray_rx_ids": json.dumps([r.rx_id for r in all_rays]),
            "ray_tx_types": json.dumps([r.tx_type for r in all_rays]),
            "ray_rx_types": json.dumps([r.rx_type for r in all_rays]),
            "ray_path_vertices": json.dumps([r.path_vertices for r in all_rays]),
            "ray_path_types": json.dumps([r.path_type for r in all_rays]),
            "ray_path_powers_db": json.dumps([round(r.path_power_db, 2) for r in all_rays]),
            "ray_path_delays_s": json.dumps([round(r.path_delay_s, 9) for r in all_rays]),
            "ray_los_flags": json.dumps([int(r.los_flag) for r in all_rays]),
        }
