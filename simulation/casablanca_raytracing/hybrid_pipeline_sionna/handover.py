...
class HandoverController:
    ...
    def _evaluate_one(self, timestamp_s: float, ue: UEState, snapshot: MeasurementSnapshot) -> HandoverDecision:
        ...
        # 1. Apply L3 Measurement Filtering (EWMA)
        alpha = 0.3
        for cell_id, meas in snapshot.by_cell_id.items():
            if cell_id in runtime.l3_rsrp:
                runtime.l3_rsrp[cell_id] = (1 - alpha) * runtime.l3_rsrp[cell_id] + alpha * meas.rsrp_dbm
                runtime.l3_sinr[cell_id] = (1 - alpha) * runtime.l3_sinr[cell_id] + alpha * meas.sinr_db
            else:
                runtime.l3_rsrp[cell_id] = meas.rsrp_dbm
                runtime.l3_sinr[cell_id] = meas.sinr_db
            meas.rsrp_dbm = runtime.l3_rsrp[cell_id]
            meas.sinr_db = runtime.l3_sinr[cell_id]
        ...
        if serving.rsrp_dbm <= self.config.rlf_rsrp_dbm and candidate is None:
            ...
            runtime.executing_target_id = None
            runtime.executing_since_s = None
        ...
        # 2. Simulate Signaling Delay (30ms for RRC Reconfiguration)
        if runtime.executing_target_id != candidate.cell_id:
            runtime.executing_target_id = candidate.cell_id
            runtime.executing_since_s = timestamp_s
            return HandoverDecision(
                source_cell_id=serving.cell_id,
                target_cell_id=serving.cell_id,
                reason="signaling_pending",
            )
        elif (timestamp_s - runtime.executing_since_s) < 0.03: # 30ms delay
            return HandoverDecision(
                source_cell_id=serving.cell_id,
                target_cell_id=serving.cell_id,
                reason="signaling_pending",
